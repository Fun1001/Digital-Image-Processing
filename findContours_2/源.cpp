...
int main()
{
	...
	cv::Mat binaryMat;
	cv::Mat dstMat;
	cv::Mat disMat = imread("E:\\3.jpg");
	cv::Mat srcMat = imread("E:\\3.jpg", 0);
	cv::threshold(srcMat, binaryMat, 0, 255, THRESH_OTSU);//二值化

	//开运算
	Mat kernel = getStructuringElement(MORPH_RECT, Size(7, 7));
	morphologyEx(binaryMat, binaryMat, MORPH_OPEN, kernel, Point(-1, -1), 1);
